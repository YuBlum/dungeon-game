...
system_move_by_input(void) {
  ...
  for (Entity e = 0; e < ecs_entities_amount(); e++) {
    ...
    if ((dir.x != 0 || dir.y != 0) && (dir.x == 0 || dir.y == 0)) {
      move_position[e].nxt = v2_add(position[e], dir);
...
system_apply_movement(void) {
  ...
  for (Entity e = 0; e < ecs_entities_amount(); e++) {
    ...
    if (move_position[e].timer >= 1.0f) {
      ...
      position[e] = v2_lerp(move_position[e].prv, move_position[e].nxt, smoothstep(0, 1, move_position[e].timer));
    }
  }
}
...
systems_create(void) {
  ...
  ecs_system_must_have(system_apply_movement, "position", "move-position");
