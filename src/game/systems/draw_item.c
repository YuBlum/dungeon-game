...
system_draw_item(usize entities_amount) {
  ...
  for (Entity e = 0; e < entities_amount; e++) {
    ...
    if (item_amount) {
      renderer_text(V2F(UI_LEFT + PX_TO_UNIT, y), 1, false, false, fg, layer, "%.*s x%u", item[e].name_size, item[e].name_buff, item_amount[e]);
    } else {
      renderer_text(V2F(UI_LEFT + PX_TO_UNIT, y), 1, false, false, fg, layer, "%.*s", item[e].name_size, item[e].name_buff);
    }
