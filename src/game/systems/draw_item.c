...
system_draw_item(usize entities_amount) {
  ...
  for (Entity e = 0; e < entities_amount; e++) {
    ...
    Color name_col = fg;
    if (equiped && equiped[e] && global.game.weapon.has_item && global.game.weapon.type == item[e].type && global.game.weapon.id == item[e].id) {
      name_col = bg == 0x000000ff ? 0x00ff00ff : 0x00aa00ff;
    }
    ...
    if (item_amount) {
      renderer_text(V2F(UI_LEFT + PX_TO_UNIT, y), 1, false, false, name_col, layer, "%.*s x%u", item[e].name_size, item[e].name_buff, item_amount[e]);
    } else {
      renderer_text(V2F(UI_LEFT + PX_TO_UNIT, y), 1, false, false, name_col, layer, "%.*s", item[e].name_size, item[e].name_buff);
    }
